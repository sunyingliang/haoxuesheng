...
if (!empty($_SERVER['HTTPS']) && ('on' == $_SERVER['HTTPS'])) {
	...
	if(in_array($pID, $partneridArray))
	{
		...
		header('Location: RedirectPage.php?uID='.$nric.'&ipRef='.$ipFrom.'&pID='.$pID.'&token='.$token.'&ref='.$ref);
		//$redirectUrl= $uri."/sys/redirect_mc_sys.php?uID=$nric&ipRef=$ipFrom&pID=$pID&token=$token&ref=$ref";
		//header ("Location: Pageredirect.php?redirect=$redirectUrl");
	}else
	{
